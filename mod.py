...
def TTlovesEmbeds(body,title=False):
    if(title==False):
        return discord.Embed(description=body, colour=0xFF0000)
    else:
        return discord.Embed(title=title, description=body, colour=0xFF0000)
...
class Mod:
    ...
    def __init__(self, bot):
        ...
        self.owners = ["173102900514521089", "152802677867282432"]
    ...
    async def modset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            msg = ("Admin role: {ADMIN_ROLE}\n"
                   ...
                   "".format(**_settings))
            await self.bot.say(embed=TTlovesEmbeds(box(msg)))
    ...
    async def _modset_adminrole(self, ctx, *, role_name: str):
        ...
        if server.id not in settings.servers:
            await self.bot.say(embed=TTlovesEmbeds("Remember to set modrole too."))
        settings.set_server_admin(server, role_name)
        await self.bot.say(embed=TTlovesEmbeds("Admin role set to '{}'".format(role_name)))
    ...
    async def _modset_modrole(self, ctx, *, role_name: str):
        ...
        server = ctx.message.server
        if server.id not in settings.servers:
            await self.bot.say(embed=TTlovesEmbeds("Remember to set adminrole too."))
        settings.set_server_mod(server, role_name)
        await self.bot.say(embed=TTlovesEmbeds("Mod role set to '{}'".format(role_name)))
    ...
    async def modlog(self, ctx, channel : discord.Channel=None):
        ...
        if channel:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Mod events will be sent to {}"
            ...
            await self.bot.say(embed=TTlovesEmbeds("Mod log deactivated."))
    ...
    async def banmentionspam(self, ctx, max_mentions : int=False):
        ...
        if max_mentions:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Autoban for mention spam enabled. "
            ...
            await self.bot.say(embed=TTlovesEmbeds("Autoban for mention spam disabled."))
        dataIO.save_json("data/mod/settings.json", self.settings)
    ...
    async def deleterepeats(self, ctx):
        ...
        if not self.settings[server.id]["delete_repeats"]:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Messages repeated up to 3 times will "
                               "be deleted."))
        else:
            self.settings[server.id]["delete_repeats"] = False
            await self.bot.say(embed=TTlovesEmbeds("Repeated messages will be ignored."))
        dataIO.save_json("data/mod/settings.json", self.settings)
    ...
    async def resetcases(self, ctx):
        ...
        await self.bot.say(embed=TTlovesEmbeds("Cases have been reset."))
    ...
    async def deletedelay(self, ctx, time: int=None):
        ...
        if time is not None:
            ...
            if time == -1:
                await self.bot.say(embed=TTlovesEmbeds("Command deleting disabled."))
            else:
                await self.bot.say(embed=TTlovesEmbeds("Delete delay set to {}"
                                   " seconds.".format(time)))
            dataIO.save_json("data/mod/settings.json", self.settings)
        else:
            try:
                ...
                await self.bot.say(embed=TTlovesEmbeds("Delete delay not yet set up on this"
                                   " server."))
            else:
                if delay != -1:
                    await self.bot.say(embed=TTlovesEmbeds("Bot will delete command messages after"
                                       " {} seconds. Set this value to -1 to"
                                       " stop deleting messages".format(delay)))
                else:
                    await self.bot.say(embed=TTlovesEmbeds("I will not delete command messages."))
    ...
    async def set_cases(self, ctx, action: str = None, enabled: bool = None):
        ...
        if action == enabled:  # No args given
            ...
            msg += '```'
            await self.bot.say(embed=TTlovesEmbeds(msg))
            ...
            value = self.settings[server.id].get(action,
                                                 default_settings[action])
            await self.bot.say(embed=TTlovesEmbeds('Case creation for %s is currently %s' %
    ...
    async def kick(self, ctx, user: discord.Member=None, *, reason: str = None):
        ...
        if user is None:
            embed = discord.Embed(title="__Kick__", description="Kicks a user with a specified reason.", colour=0x111111)
            embed.add_field(name="e.g",value=".kick @TTxFTW spamming",inline=True)
            embed.set_thumbnail(url='http://www.hotel-r.net/im/hotel/gb/the-boot-1.gif')
        ...
        try:
            ...
            if self.settings[server.id].get('kick_cases',
                ...
                await self.new_case(server,
                                    ...
                                    user=user,
                                    reason=reason)
            await self.bot.say(embed=TTlovesEmbeds("Done. That felt good."))
        except discord.errors.Forbidden:
            await self.bot.say(embed=TTlovesEmbeds("I'm not allowed to do that."))
        except Exception as e:
            print(e)

    @commands.command(no_pm=True, pass_context=True)
    @checks.admin_or_permissions(ban_members=True)
    async def ban(self, ctx, user: discord.Member=None, days: str = None, *, reason: str = None):
        ...
        if user is None:
            embed = discord.Embed(title="__Ban__", description="Bans a user with a specified reason.",colour=0x111111)
            embed.add_field(name="e.g",value=".ban @TTxFTW Being Aids.",inline=True)
            embed.set_thumbnail(url='https://img.clipartfest.com/e86738feaa73235a528cae3bb785d235_sledgehammer-vector-art-clipart-sledge-hammer_366-469.jpeg')
            await self.bot.send_message(ctx.message.channel, embed=embed)
            return
        ...
        if days < 0 or days > 7:
            await self.bot.say(embed=TTlovesEmbeds("Invalid days. Must be between 0 and 7."))
            return
        ...
        try:
            ...
            if self.settings[server.id].get('ban_cases',
                ...
                await self.new_case(server,
                                    ...
                                    user=user,
                                    reason=reason)
            await self.bot.say(embed=TTlovesEmbeds("Done. It was about time."))
        except discord.errors.Forbidden:
            await self.bot.say(embed=TTlovesEmbeds("I'm not allowed to do that."))
        except Exception as e:
            print(e)
        ...
        if can_ban:
            try:
                try:  # We don't want blocked DMs preventing us from banning
                    msg = await self.bot.send_message(user, embed=TTlovesEmbeds("You have been banned and "
                ...
                await self.bot.say(embed=TTlovesEmbeds("Done. Enough chaos."))
            except discord.errors.Forbidden:
                await self.bot.say(embed=TTlovesEmbeds("My role is not high enough to softban that user."))
    ...
    async def rename(self, ctx, user : discord.Member=None, *, nickname=""):
        ...
        if user is None:
            embed=discord.Embed(title="__Rename__", description="Changes a users nickname", colour=0xFF0000)
            embed.add_field(name="Command:", value=".rename <user> <name>\n\n__Example__:\n\n.rename @TTxFTW Tarik")
            embed.set_thumbnail(url='http://images.akamai.steamusercontent.com/ugc/37484169441106465/05EFD12617F7F2D5E4F62F3CD8D38DAEF16D5F45/')
        ...
        try:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Done."))
        except discord.Forbidden:
            await self.bot.say(embed=TTlovesEmbeds("I cannot do that, I lack the "
                "\"Manage Nicknames\" permission."))

    @commands.group(pass_context=True, no_pm=True, invoke_without_command=True)
    @checks.mod_or_permissions(administrator=True)
    async def mute(self, ctx, user : discord.Member, *, reason: str = None):
        """Mutes user in the channel/server (Default's to channel)"""
                                                                  #https://img.clipartfox.com/c299628221a023a004ae011327dcacb4_-can-mute-someone-depending-mute-person-clipart_1000-727.jpeg  Use that later.
        if ctx.invoked_subcommand is None:
    ...
    async def channel_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if overwrites.send_messages is False:
            await self.bot.say(embed=TTlovesEmbeds("That user can't send messages in this "
                               "channel."))
            return
        ...
        try:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Failed to mute user. I need the manage roles "
            ...
            if self.settings[server.id].get('cmute_cases',
                ...
                await self.new_case(server,
                                    ...
                                    user=user,
                                    reason=reason)
            await self.bot.say(embed=TTlovesEmbeds("User has been muted in this channel."))
    ...
    async def server_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if not register:
            await self.bot.say(embed=TTlovesEmbeds("That user is already muted in all channels."))
            return
        ...
        if self.settings[server.id].get('smute_cases',
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
        await self.bot.say(embed=TTlovesEmbeds("User has been muted in this server."))
    ...
    async def channel_unmute(self, ctx, user : discord.Member):
        ...
        if overwrites.send_messages:
            await self.bot.say(embed=TTlovesEmbeds("That user doesn't seem to be muted "
                               "in this channel."))
            return
        ...
        try:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Failed to unmute user. I need the manage roles"
            ...
            dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
            await self.bot.say(embed=TTlovesEmbeds("User has been unmuted in this channel."))
    ...
    async def server_unmute(self, ctx, user : discord.Member):
        ...
        if user.id not in self._perms_cache:
            await self.bot.say(embed=TTlovesEmbeds("That user doesn't seem to have been muted with {0}mute commands. "
        ...
        if user.id in self._perms_cache and not self._perms_cache[user.id]:
            del self._perms_cache[user.id]  # cleanup
        dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
        await self.bot.say(embed=TTlovesEmbeds("User has been unmuted in this server."))
    ...
    async def text(self, ctx, text: str, number: int):
        ...
        if not has_permissions:
            await self.bot.say(embed=TTlovesEmbeds("I'm not allowed to delete messages."))
            return
    ...
    async def after(self, ctx, message_id : int):
        ...
        if not is_bot:
            await self.bot.say(embed=TTlovesEmbeds("This command can only be used on bots with "
                               "bot accounts."))
            return
        ...
        if not has_permissions:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Message not found."))
            return
    ...
    async def cleanup_bot(self, ctx, number: int):
        ...
        if isinstance(prefixes, str):
            ...
            if asyncio.iscoroutine(prefixes):
                await self.bot.say(embed=TTlovesEmbeds('Coroutine prefixes not yet implemented.'))
                return
    ...
    async def reason(self, ctx, case, *, reason : str=""):
        ...
        try:
            ...
            await self.bot.say(embed=TTlovesEmbeds("That case is not yours."))
        except KeyError:
            await self.bot.say(embed=TTlovesEmbeds("That case doesn't exist."))
        except NoModLogChannel:
            await self.bot.say(embed=TTlovesEmbeds("There's no mod-log channel set."))
        except CaseMessageNotFound:
            await self.bot.say(embed=TTlovesEmbeds("Couldn't find the case's message."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("Case #{} updated.".format(case)))
    ...
    async def _blacklist_add(self, user: discord.Member):
        ...
        if user.id not in self.blacklist_list:
            ...
            await self.bot.say(embed=TTlovesEmbeds("User has been added to blacklist."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("User is already blacklisted."))
    ...
    async def _blacklist_remove(self, user: discord.Member):
        ...
        if user.id in self.blacklist_list:
            ...
            dataIO.save_json("data/mod/blacklist.json", self.blacklist_list)
            await self.bot.say(embed=TTlovesEmbeds("User has been removed from blacklist."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("User is not in blacklist."))
    ...
    async def _blacklist_clear(self):
        ...
        self.blacklist_list = []
        dataIO.save_json("data/mod/blacklist.json", self.blacklist_list)
        await self.bot.say(embed=TTlovesEmbeds("Blacklist is now empty."))
    ...
    async def _whitelist_add(self, user: discord.Member):
        ...
        if user.id not in self.whitelist_list:
            ...
            await self.bot.say(embed=TTlovesEmbeds("User has been added to whitelist." + msg))
        else:
            await self.bot.say(embed=TTlovesEmbeds("User is already whitelisted."))
    ...
    async def _whitelist_remove(self, user: discord.Member):
        ...
        if user.id in self.whitelist_list:
            ...
            dataIO.save_json("data/mod/whitelist.json", self.whitelist_list)
            await self.bot.say(embed=TTlovesEmbeds("User has been removed from whitelist."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("User is not in whitelist."))
    ...
    async def _whitelist_clear(self):
        ...
        self.whitelist_list = []
        dataIO.save_json("data/mod/whitelist.json", self.whitelist_list)
        await self.bot.say(embed=TTlovesEmbeds("Whitelist is now empty."))
    ...
    async def ignore(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
            await self.bot.say(embed=TTlovesEmbeds(self.count_ignored()))
    ...
    async def ignore_channel(self, ctx, channel: discord.Channel=None):
        ...
        if not channel:
            if current_ch.id not in self.ignore_list["CHANNELS"]:
                ...
                await self.bot.say(embed=TTlovesEmbeds("Channel added to ignore list."))
            else:
                await self.bot.say(embed=TTlovesEmbeds("Channel already in ignore list."))
        else:
    ...
    async def ignore_server(self, ctx):
        ...
        if server.id not in self.ignore_list["SERVERS"]:
            ...
            dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
            await self.bot.say(embed=TTlovesEmbeds("This server has been added to the ignore list."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("This server is already being ignored."))
    ...
    async def unignore_channel(self, ctx, channel: discord.Channel=None):
        ...
        if not channel:
            if current_ch.id in self.ignore_list["CHANNELS"]:
                ...
                dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
                await self.bot.say(embed=TTlovesEmbeds("This channel has been removed from the ignore list."))
            else:
                await self.bot.say(embed=TTlovesEmbeds("This channel is not in the ignore list."))
        else:
            if channel.id in self.ignore_list["CHANNELS"]:
                self.ignore_list["CHANNELS"].remove(channel.id)
                dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
                await self.bot.say(embed=TTlovesEmbeds("Channel removed from ignore list."))
            else:
                await self.bot.say(embed=TTlovesEmbeds("That channel is not in the ignore list."))
    ...
    async def unignore_server(self, ctx):
        ...
        if server.id in self.ignore_list["SERVERS"]:
            ...
            dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
            await self.bot.say(embed=TTlovesEmbeds("This server has been removed from the ignore list."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("This server is not in the ignore list."))
                ...
                if self.filter[server.id]:
                    ...
                    try:
                        for page in pagify(words, delims=[" ", "\n"], shorten_by=8):
                            await self.bot.send_message(author, embed=TTlovesEmbeds(page))
                    except discord.Forbidden:
                        await self.bot.say(embed=TTlovesEmbeds("I can't send direct messages to you."))
    ...
    async def filter_add(self, ctx, *words: str):
        ...
        if added:
            ...
            await self.bot.say(embed=TTlovesEmbeds("Words added to filter."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("Words already in the filter."))
    ...
    async def filter_remove(self, ctx, *words: str):
        ...
        removed = 0
        if server.id not in self.filter.keys():
            await self.bot.say(embed=TTlovesEmbeds("There are no filtered words in this server."))
            return
        for w in words:
        ...
        if removed:
            dataIO.save_json("data/mod/filter.json", self.filter)
            await self.bot.say(embed=TTlovesEmbeds("Words removed from filter."))
        else:
            await self.bot.say(embed=TTlovesEmbeds("Those words weren't in the filter."))
    ...
    async def colour(self, ctx, role: discord.Role, value: discord.Colour):
        ...
        try:
            ...
            await self.bot.say(embed=TTlovesEmbeds("I need permissions to manage roles first."))
        except Exception as e:
            print(e)
            await self.bot.say(embed=TTlovesEmbeds("Something went wrong."))
    ...
    async def edit_role_name(self, ctx, role: discord.Role, name: str):
        ...
        if name == "":
            await self.bot.say(embed=TTlovesEmbeds("Name cannot be empty."))
            return
        try:
            author = ctx.message.author
    ...
    async def names(self, user : discord.Member):
        ...
        if msg:
            ...
            await self.bot.say(embed=TTlovesEmbeds("That user doesn't have any recorded name or "
                               "nickname change."))
    ...
    async def new_case(self, server, *, action, mod=None, user, reason=None, until=None, channel=None):
        ...
        case = {
            ...
            "user_id"      : user.id,
            "reason"       : reason,
            "moderator"    : str(mod.mention) if mod is not None else None,
        ...
        try:
            msg = await self.bot.send_message(mod_channel, embed=TTlovesEmbeds(case_msg))
            case["message"] = msg.id
        except:
            pass
    ...
    async def update_case(self, server, *, case, mod=None, reason=None,
        ...
        if msg:
            await self.bot.edit_message(msg, embed=TTlovesEmbeds(case_msg))
        else:
    ...
    def format_case_msg(self, case):
        ...
        if case["reason"] is None:
            tmp["reason"] = "Reason not provided."
            #tmp["reason"] = "Type .reason %i <reason> to add it" % tmp["case"]
        ...
        case_msg = (
            "__**Case #{case}**__ | {action}\n\n\n"
            "__**User:**__ {user} ({user_id})\n\n\n"
            "__**Moderator:**__ {moderator} ({moderator_id})\n\n\n"
        ).format(**tmp)
