...
default_settings = {"PAYDAY_TIME": 3600, "PAYDAY_CREDITS": 2000,
                    "SLOT_MIN": 100, "SLOT_MAX": 1500, "SLOT_TIME": 0,
...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at Tarik's Bank."""
        ...
        try:
            ...
            await self.bot.say("{}, You already have an account at"
                               " Tarik's Bank.".format(author.mention))
    ...
    async def balance(self, ctx, user: discord.Member=None):
        ...
        if not user:
            ...
            try:
                ...
                await self.bot.say("{} You don't have an account at"
                                   " Tarik's Bank. Type `{}bank register`"
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @TTxFTW 26 - Sets 26 credits
            bank set @TTxFTW +2 - Adds 2 credits
            bank set @TTxFTW -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
    ...
    @commands.command(pass_context=True)
    async def pay(self, ctx, user: discord.Member, sum: int):
        """Transfer credits to other users"""
        author = ctx.message.author
        try:
            self.bank.transfer_credits(author, user, sum)
