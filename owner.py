...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes Tarik's global settings."""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start Tarik "
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets Tarik's prefixes for this server
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart Tarik

        Makes Tarik quit with exit code 26
    ...
    # Tarik re-wrote it. That's why it's disabled.
